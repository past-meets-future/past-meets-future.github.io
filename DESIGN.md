...
- **The mark ⊕** — usage rule (2026-07-08, after redundancy critique): **exactly two placements per page** — the nav brand and the HCOMP ⊕ CI venue lockup (where it does semantic work joining two conferences). Subpages: nav only. Never in the milestone bar, never beside the wordmark, never as decoration; footers join venue names with a plain +. Favicon uses it (browser chrome, not page). **Drawing rule (organizer feedback 2026-07-07): the cross is CONTAINED within the circle — a true circled plus ⊕. Strokes that extend beyond the circle read as a gun sight / target reticle; never draw it that way.**
...
3. **Hero** — kicker ("**The 3rd Annual Workshop** · № 1 at ACM IUI 2024 · № 2 at ASIS&T 2025" — series continuity stays above the fold, organizer feedback 2026-07-07) → wordmark → pitch opening "**Human-AI interaction for digital humanities and cultural heritage**" (full workshop scope, never just "meets cultural heritage") and ending "Last year we mapped the field's challenges. **This year we explore solutions, together.**" (never "we build solutions") → venue lockup ⊕ with both conference names LINKED + "one registration covers both" note → **four fact tiles** (When / Where / **Papers due July 30** / Notifications) → CTA pair.
