...
When two printing plates align perfectly, printers say the impression is **in register** — and the circle-and-crosshair registration mark ⊕ is how they check. That is the workshop in one glyph:

**past ⊕ future · human ⊕ AI · HCOMP ⊕ CI — two plates, one impression.**

- **The mark ⊕** — usage rule (2026-07-08, after redundancy critique): **exactly two placements per page** — the nav brand and the HCOMP ⊕ CI venue lockup (where it does semantic work joining two conferences). Subpages: nav only. Never in the milestone bar, never beside the wordmark, never as decoration; footers join venue names with a plain +. Favicon uses it (browser chrome, not page).
...
3. **Hero** — kicker → wordmark → pitch ending "Last year we mapped the field's challenges. **This year we build solutions.**" → venue lockup ⊕ → **four fact tiles** (When / Where / **Papers due July 30** / Notifications) → CTA pair.
...
6. **From Challenges to Solutions** — the 2026 theme. The CFP page renders **three** challenge cards (AI-GLAM alignment · cognitive/educational/social impact · ethics) — incentives and IP/copyright/authorship were cut by organizer decision 2026-07-08 — plus the full-width vermilion "the bridge is the method" card (humanities critique ⊕ technologist building), the "map is not a checklist" line (topics are never limited to the map), and a link to the 2025 Miro board. Native rendering; a Miro iframe embed is acceptable only on a dedicated subpage with click-to-load, never on the homepage.
7. **Call for Participation** — solutions-first framing; FOUR formats: A position paper (2-4 pp) · B published paper · C systems & prototypes (working systems, early prototypes, open-source, datasets — presented as lightning talks like all accepted work) · D new modalities (AR/VR/XR; "headsets welcome"). **All four formats visually equal — no "encouraged" emphasis (organizer directive)**. The intro is a big tent (2025-CFP tone): name the audiences (HCI, AI, the humanities, GLAM institutions; students; any career stage) and say submissions need not match the challenge map. Facts block: email submission to pastmeetsfuturehaiworkshop@gmail.com by July 30 AoE; single-blind, organizers as PC; one author attends in person; SIGACCESS access line; "unsure whether your work fits? email us and ask".
...
10. **Organizers** — six cards; names link to personal sites; each card is exactly TWO lines — role first ("Postdoctoral Fellow, HCI Institute"), institution alone on the second line ("Carnegie Mellon University") — via explicit break, never luck-of-the-wrap. Names + affiliations only, no headshots.
